...
const CONFIG = {
  DEFAULT_USER_ID: '',
  DEFAULT_PROXY_IP: '',
  SUB_CONVERTER: atob('U1VCQVBJLkNNTGl1c3Nzcy5uZXQ='),
  SUB_CONFIG: atob('aHR0cHM6Ly9yYXcuZ2l0aHVidXNlcmNvbnRlbnQuY29tL0FDTDRTU1IvQUNMNFNTUi9tYXN0ZXIvQ2xhc2gvY29uZmlnL0FDTDRTU1JfT25saW5lX01pbmlfTXVsdGlNb2RlLmluaQ=='),
  SUB_PROTOCOL: 'https',
  SUB_EMOJI: 'true',
...
class ConnectionPool {
  ...
  getKey(address, port, isSocks = false, isHttp = false) {
    return `${address}:${port}:${isSocks ? 'socks' : ''}:${isHttp ? 'http' : ''}`;
  }

  async getConnection(address, port, isSocks = false, isHttp = false) {
    const key = this.getKey(address, port, isSocks, isHttp);
    
    if (this.pool.has(key)) {
      const conn = this.pool.get(key);
      if (conn.socket.readyState === CONFIG.WS_READY_STATE_OPEN) {
    ...
    const socket = isSocks 
      ? (isHttp 
        ? await httpConnect(address, port) 
        : await socks5Connect(address, port))
    ...
    if (this.pool.size > CONFIG.CONNECTION_POOL_MAX_SIZE) {
      this.cleanup(true);
    }
  ...
  cleanup(force = false) {
    ...
    for (const [key, conn] of this.pool.entries()) {
      ...
      if (force || 
          idleTime > CONFIG.CONNECTION_IDLE_TIMEOUT || 
...
class CacheManager {
  ...
  async resolveDNS(domain) {
    ...
    const result = await resolveToIPv6(domain);
...
function initConfig(env) {
  const config = { ...CONFIG };
  config.userID = env.UUID || env.uuid || env.PASSWORD || env.pswd || config.DEFAULT_USER_ID;
  config.proxyIP = env.PROXYIP || env.proxyip || config.DEFAULT_PROXY_IP;
  config.socks5Address = env.HTTP || env.SOCKS5 || config.DEFAULT_SOCKS5_ADDRESS;
  
  if (env.KEY || env.TOKEN || (config.userID && !isValidUUID(config.userID))) {
    config.dynamicUUID = env.KEY || env.TOKEN || config.userID;
...
async function handleWebSocketUpgrade(request, config, connectionPool) {
  ...
  readableWebSocketStream.pipeTo(new WritableStream({
    async write(chunk) {
      ...
      const { addressRemote, portRemote, isUDP } = headerInfo;
      isDns = isUDP && portRemote === 53;
      
      if (!config.BAN_HOSTS.includes(addressRemote)) {
...
async function handleTCPOutbound(remoteSocket, address, port, data, webSocket, config, connectionPool) {
  try {
    ...
    const tcpSocket = await connectionPool.getConnection(
      useSocks ? config.proxyIP : address,
      port,
      useSocks,
      config.enableHttp
    );
    ...
    await remoteSocketToWS(tcpSocket, webSocket);
  } catch (err) {
    safeCloseWebSocket(webSocket);
  }
}

function shouldUseSocks5(address, config) {
  if (!config.enableSocks) return false;
  return config.GO2_SOCKS5S.some(pattern => 
    pattern === '*ttvnw.net' || 
    new RegExp(`^${pattern.replace(/\*/g, '.*')}$`, 'i').test(address)
  );
}
...
async function handleTCPOutBound(remoteSocket, addressType, addressRemote, portRemote, rawClientData, webSocket, 维列斯ResponseHeader, log, connectionPool) {
  ...
  async function connectAndWrite(address, port, socks = false, http = false) {
    ...
    const tcpSocket = socks
      ...
      : await connectionPool.getConnection(address, port);
  ...
  async function nat64() {
    if (!useSocks) {
      const nat64Proxyip = `[${await cacheManager.resolveDNS(addressRemote)}]`;
...
async function handleDNSQuery(udpChunk, webSocket, 维列斯ResponseHeader, log, connectionPool) {
  try {
    ...
    const dnsPort = 53;
    let 维列斯Header = 维列斯ResponseHeader;
    const tcpSocket = await connectionPool.getConnection(dnsServer, dnsPort);
