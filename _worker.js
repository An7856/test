    ...
    readableWebSocketStream.pipeTo(new WritableStream({
        async write(chunk) {
            ...
            if (isUDP) {
                ...
                else throw new Error('UDP代理仅对DNS(53端口)启用');
            }
        ...
        close() { log(`readableWebSocketStream已关闭`); },
        abort(reason) { log(`readableWebSocketStream已中止`, JSON.stringify(reason)); },
    })).catch((err) => log('readableWebSocketStream管道错误', err));
...
async function handleTCPOutBound(remoteSocket, addressType, addressRemote, portRemote, rawClientData, webSocket, 维列斯ResponseHeader, log) {
    ...
    async function connectAndWrite(address, port, socks = false, http = false) {
        ...
        const tcpSocket = socks ? (http ? await httpConnect(address, port, log) : await socks5Connect(addressType, address, port, log)) : connect({hostname: address, port: port});
...
function process维列斯Header(维列斯Buffer, userID) {
    ...
    function isUserIDValid(userID, userIDLow, buffer) {
        const userIDString = stringify(new Uint8Array(buffer.slice(1,17)));
    ...
    else if (command !== 1) return {hasError: true, message: `command ${command} is not support, command 01-tcp,02-udp,03-mux`};
...
function stringify(arr, offset = 0) {
    ...
    if (!isValidUUID(uuid)) throw TypeError(`生成的UUID不符合规范 ${uuid}`);
    return uuid;
}
...
async function handleDNSQuery(udpChunk, webSocket, 维列斯ResponseHeader, log) {
    try {
        ...
        await tcpSocket.readable.pipeTo(new WritableStream({
            ...
            close() { log(`DNS服务器(${dnsServer}) TCP连接已关闭`); },
            abort(reason) { console.error(`DNS服务器(${dnsServer}) TCP连接异常中断`, reason); },
        }));
    } catch (error) {
...
async function httpConnect(addressRemote, portRemote, log) {
    const {username, password, hostname, port} = parsedSocks5Address;
    const sock = await connect({hostname: hostname, port: port});
...
function socks5AddressParser(address) {
    ...
    if (former) {
        ...
        if (formers.length !== 2) throw new Error('无效的SOCKS地址格式：认证部分必须是"username:password"的形式');
        [username, password] = formers;
    }
    const latters = latter.split(":");
    port = Number(latters.pop());
    if (isNaN(port)) throw new Error('无效的SOCKS地址格式：端口号必须是数字');
    hostname = latters.join(":");
    const regex = /^\[.*\]$/;
    if (hostname.includes(":") && !regex.test(hostname)) throw new Error('无效的SOCKS地址格式：IPv6地址必须用方括号括起来，如[2001:db8::1]');
...
async function 生成配置信息(userID, hostName, sub, UA, RproxyIP, _url, fakeUserID, fakeHostName, env) {
    if (sub) {
        ...
        if ((addresses.length+addressesapi.length+addressesnotls.length+addressesnotlsapi.length+addressescsv.length) == 0) {
            ...
            if (hostName.includes("worker") || hostName.includes("notls")) {
                ...
                addressesnotls = addressesnotls.concat(cfips.map(cidr => generateRandomIPFromCIDR(cidr)+':'+randomPorts[Math.floor(Math.random()*randomPorts.length)]+'#CF随机节点'+String(counter++).padStart(2,'0')));
            } else {
                const randomPorts = httpsPorts.concat('443');
                addresses = addresses.concat(cfips.map(cidr => generateRandomIPFromCIDR(cidr)+':'+randomPorts[Math.floor(Math.random()*randomPorts.length)]+'#CF随机节点'+String(counter++).padStart(2,'0')));
            }
        }
    }
    ...
    if (userAgent.includes('mozilla') && !subParams.some(_searchParams => _url.searchParams.has(_searchParams))) {
        const newSocks5s = socks5s.map(socks5Address => socks5Address.includes('@') ? socks5Address.split('@')[1] : socks5Address.includes('//') ? socks5Address.split('//')[1] : socks5Address);
        let socks5List = '';
...
async function resolveToIPv6(target) {
    ...
    async function queryNAT64(domain) {
        const socket = connect(atob('ZG90Lm5hdDY0LmRrOjg1Mw=='), {secureTransport: 'on', allowHalfOpen: false});
        const writer = socket.writable.getWriter();
        const reader = socket.readable.getReader();
        try {
    ...
    function buildDNSQuery(domain) {
        ...
        for (const label of domain.split('.')) {
            ...
            for (let i=0;i<label.length;i++) view.setUint8(offset++,label.charCodeAt(i));
        }
    ...
    async function readDNSResponse(reader) {
        ...
        while (true) {
            ...
            if (expectedLength === null && totalLength >= 2) expectedLength = (chunks[0][0]<<8)|chunks[0][1];
    ...
    try {
        ...
        let domain;
        if (isIPv4(target)) domain = target+atob('LmlwLjA5MDIyNy54eXo=');
        else {
